...
def check_gradient_flow():
    ...
    if coef_head_grad is not None:
        ...
        if coef_head_grad.abs().max().item() < 1e-8:
            print("  [WARN]  WARNING: Gradients are vanishing!")
    else:
        print(f"  [ERROR] ERROR: No gradients computed for coefficient head!")
        return False
    ...
    if sale_head_grad is not None:
        ...
        print(f"  [ERROR] ERROR: No gradients computed for sale target head!")
        return False

    print("\n[OK] Gradients are flowing to output heads.")
    return True
...
def check_output_diversity():
    ...
    if coef_variance < 1e-6:
        print("\n[WARN]  WARNING: Coefficients are identical across different inputs!")
        print("   This suggests the model is not responsive to input state.")
        return False

    if sale_variance < 1e-6:
        print("\n[WARN]  WARNING: Sale targets are identical across different inputs!")
        print("   This suggests the model is not responsive to input state.")
        return False
...
def check_loaded_model():
    ...
    if last_run_file.exists():
        ...
        if checkpoint_path.exists():
            ...
            if unique_coefs == 1:
                print(f"\n[WARN]  LOADED MODEL HAS THE PROBLEM!")
