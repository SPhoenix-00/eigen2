...
class Actor(nn.Module):
    ...
    def forward(self, state: torch.Tensor, return_attention_weights: bool = False) -> torch.Tensor:
        ...
        # Debug: Log original values for ROCm (only during first training update, not during eval)
        if hasattr(self, '_debug_rocm') and self._debug_rocm and self.training:
            print(f"    [SymLog] Actor input: min={state.min().item():.2f}, max={state.max().item():.2f}, mean={state.mean().item():.2f}")
        
        state = symlog(state)
        
        # Debug: Log transformed values for ROCm (only during first training update, not during eval)
        if hasattr(self, '_debug_rocm') and self._debug_rocm and self.training:
