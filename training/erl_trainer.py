...
class ERLTrainer:
    ...
    def _create_dataloader(self):
        ...
        # It causes "Memory access fault by GPU node" when workers pass tensors back to main process.
        # Even with num_workers=0, pin_memory=True can cause faults on this system.
        # Solution: Disable BOTH multiprocessing and pin_memory for ROCm.
        from utils.device import get_gpu_backend
        gpu_backend = get_gpu_backend()
        use_pin_memory = True
        ...
        if gpu_backend == "ROCm":
            ...
            print(f"  [ROCm] Disabling pin_memory to prevent memory access faults")
        ...
        if num_workers == 0:
            self.replay_dataloader = DataLoader(
                ...
                num_workers=0,
                pin_memory=use_pin_memory  # Disabled for ROCm
            )
        else:
            self.replay_dataloader = DataLoader(
                self.replay_buffer,
                ...
                pin_memory=use_pin_memory,  # Faster GPU transfer
    ...
    def evaluate_population_parallel(self) -> Tuple[List[float], Dict]:
        ...
        if gpu_backend == "ROCm":  # gpu_backend defined in worker count reduction section above
    ...
    def train_population(self):
        ...
        # CRITICAL FOR ROCm: Ensure networks are in correct mode before training
        # (GPU verification removed - training is stable now)
        if gpu_backend == "ROCm" and torch.cuda.is_available() and len(self.population) > 0:
            # Silently ensure first agent networks are in correct mode
                                ...
                                for k, v in batch_cpu.items():
                                    ...
                                    if torch.isnan(v_copy).any() or torch.isinf(v_copy).any():
                                        ...
                                        if nan_ratio > 0.01:  # More than 1% NaN
                                            ...
                                            print(f"  [ROOT CAUSE DEBUG] ⚠️  ACTION CORRUPTION DETECTED!", flush=True)
                                            print(f"  [ROOT CAUSE DEBUG] This suggests the issue is in:", flush=True)
                                            print(f"  [ROOT CAUSE DEBUG]   - Agent network output (exploding gradients)", flush=True)
                                            print(f"  [ROOT CAUSE DEBUG]   - Action normalization/clipping", flush=True)
                                        elif k == 'rewards':
                                            print(f"  [ROOT CAUSE DEBUG] ⚠️  REWARD CORRUPTION DETECTED!", flush=True)
                                            print(f"  [ROOT CAUSE DEBUG] This suggests the issue is in:", flush=True)
                                            print(f"  [ROOT CAUSE DEBUG]   - Reward function (division by zero)", flush=True)
                                            print(f"  [ROOT CAUSE DEBUG]   - Reward calculation edge cases", flush=True)
